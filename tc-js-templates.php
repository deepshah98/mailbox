	...
	<dl class="tm-extra-product-options-totals tm-custom-price-totals">
		...
		<dd class="tm-final-totals">
			<span class="price amount final">
				{{{ data.formatted_final_total.replace(/(\$)(.+)/,'<span>$1</span><span>$2').split('.').join('</span><span>.') }}}</span>
			</span>
		</dd>
		<span class="no-tax-shipping"><?php dynamic_sidebar( 'widget-area-3' ); ?> <# if (parseInt(data.formatted_final_total.replace('$','').replace(',','')) >= 30) { #> + Free Shipping <# } #></span>
		<?php echo apply_filters('wc_epo_template_after_final_total','');?>
		<# } #>
